...
class ModelExtraFeatureCRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None, extra_features=None):
        outputs = self.bert(
            input_ids,
        ...
        feature = self.feature_layer(extra_features.float().view(batch_size, max_seq_length, self.num_features))
        output_layer_extra_features = torch.cat([last_encoder_layer, feature], dim=-1)
