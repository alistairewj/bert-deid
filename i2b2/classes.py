    ...
    def __init__(self, s_sas, g_sas,
        ...
        for doc_id in list(set(s_sas.keys()) & set(g_sas.keys())):
            if filters is not None:
                ...
                if conjunctive:
                    if invert:
                        gold = [t for t in self.get_tagset(g_sas[doc_id])
                                if not all([f(t) for f in self.filters])]
                        sys = [t for t in self.get_tagset(s_sas[doc_id])
                               if not all([f(t) for f in self.filters])]
                    else:
                        gold = [t for t in self.get_tagset(g_sas[doc_id])
                ...
                gold = self.get_tagset(g_sas[doc_id])
                sys = self.get_tagset(s_sas[doc_id])

            if binarize:
                # replace all unique tags with "PHI"
                for t in gold:
                    t.name = 'PHI'
                    t.TYPE = 'PHI'
                for t in sys:
...
class PHITrackEvaluation(CombinedEvaluation):
    ...
    def __init__(self, annotator_cas, gold_cas, **kwargs):
        ...
        # Binary tokenized evaluation
        self.add_eval(EvaluateTokenizedPHI(annotator_cas, gold_cas,
