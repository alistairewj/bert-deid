...
def token_type_eval(phi_token_true, tp_list, fp_list, fn_list, id2label_map, curr_performance):
    ...
    for idval in id2label_map.keys():
        curr_performance['n_{}_token_phi'.format(id2label_map[idval])] = count_freq(phi_token_true, idval)
        curr_performance['n_{}_token_tp'.format(id2label_map[idval])] = count_freq(tp_list, idval)
        curr_performance['n_{}_token_fp'.format(id2label_map[idval])] = count_freq(fp_list, idval)
        curr_performance['n_{}_token_fn'.format(id2label_map[idval])] = count_freq(fn_list, idval)
...
def main():
    ...
    parser.add_argument(
        "--stats",type=str,default=None,help="CSV file for result performance"
    )
    ...
    for fn in tqdm(input_files,total=len(input_files)):
        ...
        for i, row in df.iterrows():
            if args.bio:
                if ('B-' not in row['entity_type'] and 'I-' not in row['entity_type']) and row['entity_type'].lower() != 'phi':
                    entity_type = row['entity_type']
                if ('B-' in row['entity_type'].upper() or 'I-' in row['entity_type'].upper()):
            ...
            if args.binary_eval:
                ...
                try:
                    text_pred[row['start']:row['stop']] = label2id_map[entity_type.upper()]
                except KeyError:
                    label_map = {'age': 'AGE', 'date': 'DATE', 'email': 'CONTACT', 'idnum': 'ID',
                    ...
                    text_pred[row['start']:row['stop']] = label2id_map[transformed_label]
        ...
        if args.multi_eval:
            token_type_eval(phi_tokens_true, tp_list, fp_list, fn_list, id2label_map, curr_performance)


        fp_list_entity, fn_list_entity = None, None
        # report performance entity-wise (only when label is transformed)
        if args.bio:
            df = merge_BIO_pred(df, args.binary_eval, args.expand_eval, text, label2id_map)
            if args.binary_eval:
                gs['entity_type'] = 'phi'
                df['entity_type'] = 'phi'
    ...
    final_stats['token_macro'] = [round(se.mean(), 4), round(ppv.mean(), 4), round(f1.mean(), 4)]
    ...
    final_stats['token_micro'] = [round(se.mean(), 4), round(ppv.mean(), 4), round(f1.mean(), 4)]
    ...
    if args.multi_eval:
        ...
        for label in token_stats.keys():
            ...
            print(f'Token Micro P+: {ppv.mean():0.4f}')
            print(f'Token Micro F1: {f1.mean():0.4f}')
            final_stats[f'Token Micro {label}'] = [round(se.mean(), 4), round(ppv.mean(), 4), round(f1.mean(), 4)]
    
    if stats_path is not None:
        final_stats = pd.DataFrame.from_dict(final_stats, orient='index', columns = ['Se', 'P+', 'F1'])
