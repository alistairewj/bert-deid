...
idxKeep = [i for i in range(len(labels)) if labels[i] not in labels_to_remove]
...
for i in ordered_label_errors:
    if label_map[labels[i]] == 'STATE':
        continue

    start, stop = offsets[i], offsets[i] + lengths[i]
    print(f'{files[i]} - [{start} - {stop}] - {label_map[labels[i]]}:', end=' ')
    with open(text_path / files[i], 'r') as fp:
        text = ''.join(fp.readlines())

    text = text.replace('\n', ' ')
    print(text[max(start - 15, 0):start], end='')
    print(bcolors.FAIL, end='')
    print(text[start:stop], end='')
    print(bcolors.ENDC, end='')
    print(text[stop:min(stop + 150, len(text))], end='')
    print()
    n += 1
