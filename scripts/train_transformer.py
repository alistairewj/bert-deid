...
ALL_MODELS = sum(
    (
        tuple(conf.pretrained_config_archive_map.keys()) for conf in (
            BertConfig, RobertaConfig, DistilBertConfig, CamembertConfig,
            XLMRobertaConfig
        )
    ),
    (),
...
def argparser():
    ...
    _PATTERN_NAMES = (
        'age', 'date', 'email', 'idnum', 'initials', 'location', 'mrn', 'name',
        'pager', 'ssn', 'telephone', 'unit', 'url', 'all'
    )
    parser.add_argument(
        "--feature",
        type=str,
...
def main():
    ...
    if (
        os.path.exists(args.output_dir) and os.listdir(args.output_dir) and
    ...
    # Saving best-practices: if you use defaults names for the
    # model you can reload it using from_pretrained()
